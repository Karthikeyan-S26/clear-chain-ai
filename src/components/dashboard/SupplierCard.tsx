...
export function SupplierCard({ supplier, className }: SupplierCardProps) {
  ...
  return (
    <div className={cn("glass-card-hover p-5 space-y-4", className)}>
      <div className="flex items-start justify-between">
        <div className="space-y-1">
          <div className="flex items-center gap-2">
            <h3 className="font-semibold text-foreground">{supplier.name}</h3>
            {supplier.phantomDetected && (
              <Ghost className="w-4 h-4 text-risk-phantom animate-pulse" />
            )}
          </div>
        ...
        <div className="flex gap-2">
          <span className={cn("px-2 py-1 text-xs font-medium rounded-md border", tierStyles[supplier.tier])}>
            Tier {supplier.tier}
          </span>
          <span className={cn("px-2 py-1 text-xs font-medium rounded-md border capitalize", riskStyles[supplier.riskLevel])}>
            {supplier.riskLevel}
          </span>
        </div>
      </div>
      ...
      {supplier.phantomDetected && (
        <div className="flex items-center gap-2 p-3 rounded-lg bg-risk-phantom/10 border border-risk-phantom/20">
          <AlertTriangle className="w-4 h-4 text-risk-phantom" />
